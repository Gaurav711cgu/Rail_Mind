...
async def test_cascade_predictor_runs_bfs():
    ...
    state = {
        ...
        "trains": [
            {"train_no": "22415", "current_station": "GZB", "current_delay": 0, "status": "RUNNING", "train_type": "SUPERFAST"},
        ],
    }
    updates, confidence, reasoning = await agent.process(state)
    assert confidence > 0.0
...
async def test_dispatch_agent_generates_recommendation():
    ...
    state = {
        ...
        "trains": [
            {"train_no": "12002", "current_station": "NDLS", "current_delay": 40, "status": "HELD",    "train_type": "SUPERFAST"},
            {"train_no": "BOXN-902", "current_station": "GZB", "current_delay": 5, "status": "RUNNING", "train_type": "FREIGHT"},
        ],
        "recommendations": [],
    }
    updates, confidence, reasoning = await agent.process(state)
