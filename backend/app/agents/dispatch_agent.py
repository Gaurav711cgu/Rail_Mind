...
class DispatchAgent(BaseAgent):
    ...
    async def process(self, state: Dict[str, Any]) -> Tuple[Dict[str, Any], float, str]:
        ...
        if settings.GROQ_API_KEY:
            try:
                self.log(f"Querying Groq LLM ({settings.GROQ_MODEL}) for operational resolution...")
                prompt = f"""
...
"""
                ...
                payload = {
                    ...
                    "messages": [
                        {"role": "system", "content": "You are a precise railway operations dispatcher. Output valid JSON only."},
                ...
                async with httpx.AsyncClient(timeout=15.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        new_recommendation = {
                            ...
                            "target_section": parsed_rec.get("target_section", "GZB-ALJN loop line"),
                            "reasoning": parsed_rec.get("reasoning", "Hold Coal Freight to clear high-priority path."),
                            "confidence": rec_confidence,
                            "tier": 1 if rec_confidence >= self.auto_execute_threshold else 2,
                            "is_approved": False
                        }
                        self.log("Successfully generated recommendation using Groq LLM.")
                    else:
                        self.log(f"Groq API returned error status {response.status_code}. Using fallback heuristic.")
            except Exception as e:
                self.log(f"Error querying Groq API: {str(e)}. Using fallback heuristic.")
        else:
